...
#PNG folder (MAP)
output_dir_maps_PNG  <-
  paste0(output_dir_maps, "/PNG_SP_CC")
if (!dir.exists(output_dir_maps_PNG)) {
  dir.create(output_dir_maps_PNG)
}
#TIF folder
output_dir_maps_TIF  <-
  paste0(output_dir_maps, "/TIF_SP_CC")
...
output_dir_maps_AREAS_potential  <-
  paste0(output_dir_maps, "/changes_SP_CC")
if (!dir.exists(output_dir_maps_AREAS_potential)) {
  dir.create(output_dir_maps_AREAS_potential)
}

#change folder calculation (realized niche concave hull)
output_dir_maps_AREAS_ConcaveHull  <-
  paste0(output_dir_maps, "/changes_SP_ConcaveHul")
if (!dir.exists(output_dir_maps_AREAS_ConcaveHull)) {
  dir.create(output_dir_maps_AREAS_ConcaveHull)
}
#change folder calculation (realized niche convex hull)
output_dir_maps_AREAS_ConvHull  <-
  paste0(output_dir_maps, "/changes_SP_ConvHull")
if (!dir.exists(output_dir_maps_AREAS_ConvHull)) {
  dir.create(output_dir_maps_AREAS_ConvHull)
}
...
for(i in 1:2){
  SSP_dir <- paste0(output_dir_maps_TIF,"/",SSPs[[i]])
  if(!dir.exists(SSP_dir)){
    dir.create(SSP_dir)
  }
  SSP_dir_PNG <- paste0(output_dir_maps_PNG,"/",SSPs[[i]])
  if(!dir.exists(SSP_dir_PNG)){
    dir.create(SSP_dir_PNG)
  }
  SSP_dir_Areas_pot <- paste0(output_dir_maps_AREAS_potential,"/",SSPs[[i]])
  if(!dir.exists(SSP_dir_Areas_pot)){
    dir.create(SSP_dir_Areas_pot)
  }
  SSP_output_dir_maps_AREAS_ConcaveHull <- paste0(output_dir_maps_AREAS_ConcaveHull,"/",SSPs[[i]])
  if(!dir.exists(SSP_output_dir_maps_AREAS_ConcaveHull)){
    dir.create(SSP_output_dir_maps_AREAS_ConcaveHull)
  }
  SSP_output_dir_maps_AREAS_ConvHull <- paste0(output_dir_maps_AREAS_ConvHull,"/",SSPs[[i]])
  if(!dir.exists(output_dir_maps_AREAS_ConvHull)){
    dir.create(output_dir_maps_AREAS_ConvHull)
  }
...
##############################################################################
#input folder
FutDir <- paste0(resultdir,"/Distribution maps/Future/2050/Consensus_maps_folder2/Fut_consensus")
...
for(s in 1:2){
  ...
  for(i in seq_along(avail_sp_modeled)){
    ...
    message(paste0(SSPs[[s]]," / ",sp_name," / ",i))
    out_path <- paste0(FutDir, "/", sp_name,"_",SSPs[[s]],".tif")
    
    if(file.exists(out_path)){
      out_path_current <- paste0(InDir, "/", sp_name,".tif")
      # out_path <- paste0(output_dir_maps_TIF, "/", sp_name,"_",SSPs[[s]], ".tif")
      if(!file.exists(paste0(output_dir_maps_TIF,"/",SSPs[[s]],"/",sp_name,"_changes",".tif"))){
        
        future <- terra::rast(out_path)
        future <- terra::resample(future,template,"near")
        current <- terra::rast(out_path_current)
        current <- terra::resample(current,template,"near")
        
        future[which(future[]==1)] <- 2
        
        change <- future-current
        setwd(dir = paste0(output_dir_maps_TIF,"/",SSPs[[s]]))
        terra::writeRaster(change,paste0(sp_name,"_changes",".tif"),overwrite=T)
        
      } else {
        change <- terra::rast(paste0(output_dir_maps_TIF,"/",SSPs[[s]],"/",sp_name,"_changes",".tif"))
      }
      ###########################
      #get areas for changes
      x_Area <- terra::expanse(change,unit="km",byValue=T,wide=T)
      ...
      write.csv(area_df,paste0(output_dir_maps_AREAS_potential,"/",SSPs[[s]],"/",sp_name,"_areas",".csv"))
      ...
      setwd(dir = paste0(output_dir_maps_PNG,"/",SSPs[[s]]))
