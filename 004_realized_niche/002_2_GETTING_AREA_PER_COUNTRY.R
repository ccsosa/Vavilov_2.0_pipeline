...
areas_per_country <-
  function(resultdir,
    ...
    output_dir_maps_AREAS_potential  <-
      paste0(output_dir_maps, "/changes_SP_CC_COUNTRY")
      ...
      for (s in 1:2) {
        ...
        for (i in seq_along(avail_sp_modeled)) {
          ...
          if (file.exists(out_path)) {
            ...
            for (j in 1:nrow(adm0)) {
              
              x_j <- terra::mask(x, adm0[j, ])
              x_j <- terra::crop(x_j, adm0[j, ])
