### this script can be used to get summary richness maps using realized niche tif files in several categories for future projections
...
# set target region
summary_richness_function_FUTURE_realized <-
  function(basedir,
           column_name,
           resultdir,
           template_file,
    ...
    out_buffer_dir_summary_png <-
      paste0(out_buffer_dir_summary, "/", "PNG_future_realized")
    out_buffer_dir_summary_tif <-
      paste0(out_buffer_dir_summary, "/", "TIF_future_realized")
    out_buffer_dir_summary_html <-
      paste0(out_buffer_dir_summary, "/", "HTML_future_realized")
    ...
    out_buffer_dir_summary_chunks <-
      paste0(out_buffer_dir_summary, "/chunks_F_R")
    ...
    InDir <-
      "//catalogue/MultifLandscapesA1706/1.Data/Results/Summary_maps_folder_CC/TIF_SP_REALIZED_SDM/ConcaveHull"
        ...
        if (!file.exists(
          ...
          valid_paths <-
            vapply(avail_sp_modeled_i$species, function(sp) {
              path <- paste0(InDir, "/",SSPs[[s]],"/",sp, ".tif")
              if (file.exists(path))
                path
              else
                NA_character_
...
column_name <- "wild"
summary_richness_function_FUTURE_realized(basedir,
                          column_name,
                          resultdir,
                          template_file,
                          species_selected)
